...
export default function Loads({ loads, setLoads, api, showToast, fetchLoads }) {
  ...
  function calcPay(load) {
    ...
    return { basePay, bruceGross: basePay * BRUCE_CUT, timGross: basePay * TIM_CUT, fuel: parseFloat(load.fuel || 0) }
  }
          ...
          {filteredLoads.map((load, idx) => {
            ...
            return (
              <div key={load.id || idx} style={{
                ...
                {/* ── DARK HEADER STRIP ── */}
                <div style={{
                  background: isBruce ? '#0d1e33' : '#1a0a0a',
                  padding: '10px 14px',
                  display: 'flex',
                  alignItems: 'center',
                  ...
                  <span style={{
                    ...
                    fontFamily:'var(--font-head)', fontWeight:700,
                    background: load.status==='paid' ? '#1b5e20' : load.status==='billed' ? '#0d47a1' : '#333',
                    color: load.status==='paid' ? '#a5d6a7' : load.status==='billed' ? '#90caf9' : '#aaa',
                  }}>
                ...
                <div style={{ background:'#ffffff', padding:'14px 16px' }}>
                  ...
                  {/* BROKER */}
                  <div style={{ fontSize:18, fontWeight:800, color:'#111', marginBottom:4, lineHeight:1.2 }}>
                  ...
                  {/* NET BILLABLE — big and prominent */}
                  <div style={{
                    background:'#f8f8f8', borderRadius:8, padding:'10px 14px',
                    marginBottom:10, border:'1px solid #e0e0e0',
                    display:'flex', justifyContent:'space-between', alignItems:'center',
                  }}>
                    <span style={{ fontSize:13, fontWeight:700, color:'#555', fontFamily:'var(--font-head)' }}>NET BILLABLE</span>
                    <span style={{ fontSize:26, fontWeight:900, color:'#1a6b1a', fontFamily:'var(--font-head)' }}>{fmt(netPayVal)}</span>
                  </div>

                  {/* DRIVER PAY BREAKDOWN — white section */}
                  {load.driver === 'TIM' && (
                    <div style={{ background:'#f0f4ff', borderRadius:8, padding:'12px 14px', marginBottom:10, border:'1px solid #c5d5f0' }}>
                      <div style={{ fontSize:11, color:'#555', fontFamily:'var(--font-head)', letterSpacing:'0.08em', marginBottom:8, fontWeight:700 }}>
                        DRIVER PAY — THIS LOAD
                      </div>
                      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                        <span style={{ fontSize:13, color:'#555' }}>Base Rate</span>
                        <span style={{ fontSize:13, fontWeight:700, color:'#222' }}>{fmt(pay.basePay)}</span>
                      </div>
                      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8 }}>
                        <span style={{ fontSize:13, color:'#555' }}>Bruce 20% cut</span>
                        <span style={{ fontSize:13, fontWeight:700, color:'#1565c0' }}>{fmt(pay.bruceGross)}</span>
                      </div>
                      <div style={{ borderTop:'1px solid #c5d5f0', paddingTop:8, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                        <span style={{ fontSize:14, fontWeight:800, color:'#222', fontFamily:'var(--font-head)' }}>TIM GROSS PAY</span>
                        <span style={{ fontSize:22, fontWeight:900, color:'#1a6b1a', fontFamily:'var(--font-head)' }}>{fmt(pay.timGross)}</span>
                      </div>

                      {/* FUEL */}
                      <div style={{ marginTop:10, borderTop:'1px solid #c5d5f0', paddingTop:10 }}>
                        <div style={{ fontSize:10, color:'#888', fontFamily:'var(--font-head)', letterSpacing:'0.06em', marginBottom:6 }}>FUEL COST (TRACKING ONLY)</div>
                        <div style={{ display:'flex', gap:8 }}>
                          <input
                            ...
                            style={{
                              ...
                              color:'#222', borderRadius:8, padding:'8px 12px',
                          ...
                          <button
                            ...
                            style={{
                              ...
                              fontSize:12, fontFamily:'var(--font-head)', fontWeight:900, cursor:'pointer', flexShrink:0,
                            }}
                          >SAVE</button>
                        </div>
                      </div>
                  ...
                  {load.driver === 'BRUCE' && (
                    <div style={{ background:'#f0f4ff', borderRadius:8, padding:'12px 14px', marginBottom:10, border:'1px solid #c5d5f0' }}>
                      <div style={{ fontSize:11, color:'#555', fontFamily:'var(--font-head)', letterSpacing:'0.08em', marginBottom:8, fontWeight:700 }}>OWNER PAY — THIS LOAD</div>
                      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                        <span style={{ fontSize:13, color:'#555' }}>Base Rate</span>
                        <span style={{ fontSize:13, fontWeight:700, color:'#222' }}>{fmt(pay.basePay)}</span>
                      </div>
                      <div style={{ borderTop:'1px solid #c5d5f0', marginTop:6, paddingTop:8, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                        <span style={{ fontSize:14, fontWeight:800, color:'#222', fontFamily:'var(--font-head)' }}>BRUCE GROSS CUT</span>
                        <span style={{ fontSize:22, fontWeight:900, color:'#1565c0', fontFamily:'var(--font-head)' }}>{fmt(pay.bruceGross)}</span>
                      </div>
                    </div>
                  )}
                  ...
                  {parseFloat(load.comdata_total || load.comdataTotal || 0) > 0 && (
                    <div style={{
                      ...
                      <span style={{ fontSize:11, color:'#666', fontFamily:'var(--font-head)', fontWeight:700 }}>ADVANCE KEPT</span>
                      <span style={{ fontSize:16, fontFamily:'var(--font-head)', fontWeight:900, color: kept > 0 ? '#b8860b' : '#999' }}>{fmt(kept)}</span>
                    </div>
                  )}
                  ...
                  <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginTop:4 }}>
                    {load.status !== 'billed' && load.status !== 'paid' && (
                      <button
                        ...
                        style={{ flex:1, padding:'10px 12px', fontSize:13, opacity: isUpdating ? 0.5 : 1 }}
                        disabled={isUpdating}
