...
export default function App() {
  ...
  const [pin,       setPin]       = useState(null)   // held in memory only — never stored
  ...
  function logout() {
    setDriver(null)
    setPin(null)       // clear PIN from memory on logout
    setPinInput('')
    setPinDriver(null)
    setPinError(false)
    resetLoad()
  ...
  async function handlePinKey(key) {
    ...
    if (next.length === 4) {
      try {
        ...
        if (data.ok) {
          setDriver(pinDriver)
          setPin(next)       // store PIN in memory for authenticated R2 requests
          setPinInput('')
          setPinDriver(null)
          setPinError(false)
        } else {
